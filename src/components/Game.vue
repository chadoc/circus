<template>
  <div>
    ...
    <select id="animationSelector" v-model="playerStateKey">
...
const staggerFrames = 4 // define speed of animation/game
...
function animationFrame(gameFrame: number): AnimationFrame {
  let position = Math.floor(gameFrame/staggerFrames) % maxFrame.value
  return {
    x: colWidth * position,
    y: frameY.value,
    width: colWidth,
